...
public class RockerView extends View {
    ...
    private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        // 回调 返回参数
        callBack(angle, lenXY<regionRadius?lenXY:regionRadius);
     ...
     * @param angle 摇动角度
     */
    private void callBack(double angle,double radian) {
        if (null != mOnAngleChangeListener) {
            mOnAngleChangeListener.angle(angle,radian);
        }
        if (null != mOnShakeListener) {
    ...
    public interface OnAngleChangeListener {
         ...
         */
        void angle(double angle,double radian);
